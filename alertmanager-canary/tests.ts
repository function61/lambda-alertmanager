import { handler_ } from './index';
import { ActionInterface } from './actions';

class TestMockActions implements ActionInterface {
	alerts: { subject: string; details: string }[] = [];
	logMessages: string[] = [];
	...
	httpGetBody(url: string): Promise<string> {
		switch (url) {
			...
			case 'https://this-one-fails.com/':
				return Promise.resolve(
					'they body you need, but not the body you deserve'
				);
...
handler_(testMockActions).then(
	() => {
		...
		assertEqual(
			logs[1],
			'✗  https://this-one-fails.com/ @ 0ms => find<will not be found> NOT in body<they body you need, but not the body you deserve>'
		);
		assertEqual(
			logs[2],
		...
		assertEqual(
			alerts[0].details,
			'find<will not be found> NOT in body<they body you need, but not the body you deserve>'
		);
